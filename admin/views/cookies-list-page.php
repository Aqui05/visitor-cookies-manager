...
<div class="wrap vcm-admin-page">
    ...
    <p class="search-box">
        <label class="screen-reader-text" for="media-search-input">
            <?php esc_html_e('Rechercher...', 'visitor-cookies-manager'); ?>
        </label>
        <input type="text" id="media-search-input" class="search" name="s" 
               placeholder="<?php _e('Rechercher...', 'visitor-cookies-manager'); ?>" 
               value="<?php echo esc_attr(isset($_GET['s']) ? $_GET['s'] : ''); ?>">
        <input id="search-submit" type="submit" class="button" value="<?php esc_attr_e('Filtrer', 'visitor-cookies-manager'); ?>">
    </p>
</div>
...
<!-- Empecher une redirection après le bulk action: Exporter la sélection -->
    <form method="get">
        <input type="hidden" name="page" value="visitor-cookies-manager">
        <?php 
